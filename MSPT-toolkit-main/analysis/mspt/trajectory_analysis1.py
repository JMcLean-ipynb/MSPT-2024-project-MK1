...
def calc_particle_number_detected(df, csv_file):
    ...
    '''
    # Normalize the csv_file path and ensure it starts with a leading slash if missing
    if not csv_file.startswith('/'):
        csv_file = '/' + csv_file
    csv_file = os.path.normpath(csv_file)

    # Debugging statement to show the corrected path
    print("Corrected path:", csv_file)
    
    # Getting the directory of the file
    detections_folder = os.path.dirname(csv_file)
    print("Directory being checked:", detections_folder)

    # List all files in the directory
    try:
        files_in_directory = os.listdir(detections_folder)
    except Exception as e:
        print("Error accessing directory:", e)
        return None
    
    # Filter for CSV files
    detections_csv = [file for file in files_in_directory if file.endswith('.csv')]
    print("CSV files found:", detections_csv)

    # Check for multiple or no CSV files
    if len(detections_csv) != 1:
        print("Error: No or multiple detection CSV files found in", detections_folder)
        return None

    # Read the detected CSV file
    dets_df = pd.read_csv(os.path.join(detections_folder, detections_csv[0]), usecols=['frame', 'contrast'])
